...
export function storeBetDice(src: BetDice) {
    return (builder: Builder) => {
        const b_0 = builder;
        b_0.storeUint(33, 32);
...
export function loadBetDice(slice: Slice) {
    const sc_0 = slice;
    if (sc_0.loadUint(32) !== 33) { throw Error('Invalid prefix'); }
...
export function dictValueParserBetDice(): DictionaryValue<BetDice> {
    return {
        serialize: (src, builder) => {
            builder.storeRef(beginCell().store(storeBetDice(src)).endCell());
        },
        parse: (src) => {
            return loadBetDice(src.loadRef().beginParse());
        }
    }
}

export type RevealDice = {
    $$type: 'RevealDice';
    nonce: bigint;
    serverSeed: Cell;
    guess: bigint;
...
export function loadRevealDice(slice: Slice) {
    const sc_0 = slice;
    if (sc_0.loadUint(32) !== 34) { throw Error('Invalid prefix'); }
    const _nonce = sc_0.loadUintBig(64);
...
export function dictValueParserRevealDice(): DictionaryValue<RevealDice> {
    return {
        serialize: (src, builder) => {
            builder.storeRef(beginCell().store(storeRevealDice(src)).endCell());
        },
        parse: (src) => {
            return loadRevealDice(src.loadRef().beginParse());
        }
    }
}
...
export function storeDiceResult(src: DiceResult) {
    return (builder: Builder) => {
        const b_0 = builder;
        b_0.storeUint(36, 32);
...
export function loadDiceResult(slice: Slice) {
    const sc_0 = slice;
...
export function dictValueParserDiceResult(): DictionaryValue<DiceResult> {
    return {
        serialize: (src, builder) => {
            builder.storeRef(beginCell().store(storeDiceResult(src)).endCell());
        },
        parse: (src) => {
            return loadDiceResult(src.loadRef().beginParse());
        }
    }
}
...
export function dictValueParserDiceGame$Data(): DictionaryValue<DiceGame$Data> {
    return {
        serialize: (src, builder) => {
            builder.storeRef(beginCell().store(storeDiceGame$Data(src)).endCell());
        },
        parse: (src) => {
            return loadDiceGame$Data(src.loadRef().beginParse());
        }
    }
}
...
const DiceGame_getters: ABIGetter[] = [
]

export const DiceGame_getterMapping: { [key: string]: string } = {
}
...
export class DiceGame implements Contract {
    ...
    static fromAddress(address: Address) {
        return new DiceGame(address);
    }
    
    readonly address: Address; 
    readonly init?: { code: Cell, data: Cell };
    readonly abi: ContractABI = {
        types:  DiceGame_types,
        getters: DiceGame_getters,
        receivers: DiceGame_receivers,
        errors: DiceGame_errors,
    };
